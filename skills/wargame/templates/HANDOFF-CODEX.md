...
7. At any abort condition: stop, write current state to the ledger entry, hand back. Never improvise past an abort. One exception: a route patched mid-execution by its own wargamer is a resume order from the current move, not an abort.
